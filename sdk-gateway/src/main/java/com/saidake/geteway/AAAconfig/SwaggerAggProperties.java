...
@Setter
@Getter
@ConfigurationProperties("sdk.swagger-agg")
@RefreshScope
public class SwaggerAggProperties {
    ...
    public boolean isShow(String route) {
        ...
        if(generateRoutesSize > 0 && !generateRoutes.contains(route)) {   //必须在定义的generate-routes内
            return false;
        }

        if(ignoreRoutesSize > 0 && ignoreRoutes.contains(route)) {     //忽略就跳过
            return false;
        }
