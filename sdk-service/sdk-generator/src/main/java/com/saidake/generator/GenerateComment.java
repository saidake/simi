...
public class GenerateComment {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final Boolean[] hasSummaryTitle = new Boolean[]{false};   // 是否开始检测 注释行
        FileUtil.readAndWriteFile(TARGET_FILE_PATH, TARGET_FILE_PATH, currentLine -> {
            ...
            boolean isFirstFunctionStart=false;              // 函数首行开始标记
            //B. 当注释开始时，检测下方首行函数
            ...
            if(checkCommentLineFlag[0]&&normalFunctionMatcher.find()&&!isSearchFunction){
                ...
                log.info("return mark, and it is not the function to look for");
                return FileUtil.SDK_RETURN_MARK_TAG+checkCommentLine[0];
            }
...
//                    log.info("function already done, commentStorage: {}",commentStorage);
                    ...
                    String paramString = functionLine[0].replaceAll(".*?\\(", "").replaceAll("\\).*?$", "");
                    ...
                    if(!hasEndComment[0]){
                        ...
                        log.info("return mark, no comment");
                        ...
                        handleAppendComment(stringBuilder,commentStorage,emptyStarString);
                        appendToStringBuilder(stringBuilder, emptyStarString);
                        log.info("return mark, already has comment and no function summary");
                        return FileUtil.SDK_RETURN_MARK_TAG+stringBuilder;
                    }else{
                    //C. 有注释，并且有function summary
                        log.info("return mark, already has comment and function summary");
